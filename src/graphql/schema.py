...
from src.graphql.resolvers.snp_resolver import (
    get_annotations,
    scroll_annotations_,
...
from src.utils import (
    get_selected_fields,
    get_sub_selected_fields,
    get_aggregation_fields,
    get_original_field_name_or_self,
)
...
def transform_agg_fields(
    aggregation_fields: list[tuple[str, list[str]]],
) -> list[tuple[str, list[str]]]:
    """
    Transform aggregation field names from cleaned API names to original ES names.
    Uses the centralized FieldNameMapper.
    """
    transformed = []
...
class Query:
    ...
    @strawberry.field
    async def scroll_annotations(
        self, info: Info, scroll_id: Optional[str] = None
    ) -> ScrollSnp:
    ...
    @strawberry.field
    async def get_aggs_by_chromosome(
        self,
        info: Info,
        chr: str,
        start: int,
        ...
        return await search_by_chromosome(
            ...
            QueryType.AGGS,
            transform_agg_fields(aggregation_fields),
            page_args,
            transform_filter_args(filter_args),
            histogram,
        )
    ...
    @strawberry.field
    async def download_SNPs_by_chromosome(
        self,
        chr: str,
        start: int,
        end: int,
    ...
    @strawberry.field
    async def get_aggs_by_gene_product(
        self,
        info: Info,
        gene: str,
        page_args: Optional[PageArgs] = None,
    ...
    async def get_SNPs_by_keyword(
        ...
        return await search_by_keyword(
            transform_fields(fields), keyword, query_type, None, page_args
        )

    @strawberry.field
    async def get_aggs_by_keyword(
        self,
        info: Info,
        keyword: str,
        page_args: Optional[PageArgs] = None,
    ...
    @strawberry.field
    async def download_SNPs_by_keyword(
        self, keyword: str, fields: list[str], page_args: Optional[PageArgs] = None
    ) -> str:
        return await search_by_keyword(
            transform_fields(fields), keyword, QueryType.DOWNLOAD, None, page_args
        )
