...
async def search_by_chromosome(
    ...
    try:
        ...
        return await query_retrieve_all_res(
            es_fields, resp, page_args.size, STANDARD_PAGE_SIZE
        )
...
async def search_by_IDs(
    es_fields: list[str], ids: list[str], page_args=PageArgs, filter_args=FilterArgs
):
    """
...
async def query_retrieve_all_res(
    es_fields: list[str], resp: dict, page_size: int, standard_page_size: int
):
    """
    Common return function for all queries
    ...
    while len(resp["hits"]["hits"]):
        ...
        resp = await es.scroll(scroll_id=scroll_id, scroll="2s")
